{application,project,
             [{applications,[kernel,stdlib,elixir,logger,runtime_tools,bamboo,
                             jason,argon2_elixir,csv,phoenix_pubsub,
                             i18n_translations,myxql,ecto_sql,secure_random,
                             guardian]},
              {description,"project"},
              {modules,['Elixir.Project','Elixir.Project.Application',
                        'Elixir.Project.Carts',
                        'Elixir.Project.DeliveryAddressContext',
                        'Elixir.Project.DeliveryAddressContext.DeliveryAddress',
                        'Elixir.Project.Email','Elixir.Project.Mailer',
                        'Elixir.Project.OrderContext',
                        'Elixir.Project.OrderContext.Order',
                        'Elixir.Project.ProductContext',
                        'Elixir.Project.ProductContext.Product',
                        'Elixir.Project.Repo','Elixir.Project.UserContext',
                        'Elixir.Project.UserContext.ApiKey',
                        'Elixir.Project.UserContext.User',
                        'Elixir.Project.Workers.CartAgent',
                        'Elixir.Project.Workers.CartSupervisor']},
              {registered,[]},
              {vsn,"0.1.0"},
              {mod,{'Elixir.Project.Application',[]}}]}.
